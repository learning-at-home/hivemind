...
This tutorial will walk you through the steps to set up ALBERT-large-v2 collaborative training with WikiText103 dataset. It uses huggingface `datasets` and `transformers` libraries to do compute the local updates, using `hivmeind.CollaborativeOptimizer` to exchange information between peers.
...
- Run the first DHT peer to welcome trainers and record training statistics (e.g. loss, performance):
   - In this example, we use [wandb.ai](https://wandb.ai/site) to plot training metrics; If you're unfamiliar with wandb, here's a [quickstart tutorial](https://docs.wandb.ai/quickstart).
