...
class RemoteMixtureOfExperts(nn.Module):
    ...
    def forward(
    ...
    ) -> Tuple[List[List[RemoteExpert]], torch.Tensor]:
        """
        ...
        batch_outputs = []
        for output_async, expert_logits in zip(
            batch_outputs_async, batch_expert_logits
        ):
    ...
    def _run_experts(
        self, experts: List[RemoteExpert], *args, **kwargs
    ) -> Dict[RemoteExpert, torch.Tensor]:
        future_to_expert = {
            run_in_background(expert, *args, **kwargs): expert for expert in experts
        }
    ...
    def _score_experts(
        self, grid_scores: List[torch.Tensor], experts: List[List[RemoteExpert]]
        ...
        for i, expert in enumerate(flat_experts):
            expert_indices = expert.uid[
                len(self.uid_prefix) + len(self.network.UID_DELIMETER) :
            ]
            expert_indices = list(
        ...
        for batch_i, expert, score in zip(
            check_numpy(flat_batch_indices), flat_experts, flat_scores
        ):
