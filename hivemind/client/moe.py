...
class RemoteMixtureOfExperts(nn.Module):
    ...
    def forward(self, input: torch.Tensor, *args: torch.Tensor, **kwargs: torch.Tensor):
        ...
        if self.allow_broadcasting and input.ndim != 2:
            ...
            kwargs_flat = {
                key: tensor.view(-1, tensor.shape[len(flattened_dims):]) for key, tensor in kwargs.items()}
        ...
        averaged_outputs_flat = map(torch.cat, zip(
            *map_with_parallel_backward(_RemoteMoECall, *batch_jobs_args)))
    ...
    def beam_search(self, grid_scores: List[torch.Tensor], k_best: int, **kwargs) -> List[List[RemoteExpert]]:
        ...
        delimeters = np.array(self.dht.UID_DELIMETER)[None, None, None]
        ...
        for dim_index, dim_scores in enumerate(grid_scores):
            ...
            new_candidates = beam[:, :, None] + \
                delimeters + dim_indices[None, None, :]
            ...
            top_alive_lookups = [
                run_in_background(self.dht.first_k_active,
                                  cands[order], k_best, **kwargs)
            ...
            top_alive_prefixes = [result.result()
                                  for result in top_alive_lookups]
        ...
        unique_experts_by_uid = {
            expert.uid: expert for expert in unique_experts if expert != self.expert_padding}
    ...
    def compute_expert_scores(
        ...
        expert_strides = torch.cumsum(torch.as_tensor(
            [0] + expert_counts, device=grid_scores[0].device), dim=-1)[:-1]
        flat_batch_indices = (expert_index_in_batch >= expert_strides[:, None]).to(
            torch.int32).sum(0) - 1
        flat_local_indices = expert_index_in_batch - \
            expert_strides[flat_batch_indices]
        ...
        for i, expert in enumerate(flat_experts):
            expert_indices = expert.uid[len(
                self.uid_prefix) + len(self.dht.UID_DELIMETER):]
            expert_indices = list(
                map(int, expert_indices.split(self.dht.UID_DELIMETER)))
            grid_indices[i] = expert_indices

        scores_per_dim = [
            dim_scores[flat_batch_indices, dim_indices] if len(
                flat_batch_indices) else torch.zeros(0)
            for dim_scores, dim_indices in zip(grid_scores, grid_indices.T)]
        flat_scores = torch.sum(torch.stack(scores_per_dim, dim=0), dim=0)

        scores = torch.full((batch_size, max_num_experts),
                            fill_value=-float('inf'), device=grid_scores[0].device)
        # backprop-able w.r.t. flat_scores
        scores[flat_batch_indices, flat_local_indices] = flat_scores
        return scores
    ...
    def outputs_schema(self):
        if self._outputs_schema is None:
            ...
            dummy_scores = self.proj(torch.randn(
                1, self.proj.in_features)).split_with_sizes(self.grid_size, dim=-1)
            self._outputs_schema = self.beam_search(dummy_scores, k_best=1)[
                0][0].info['outputs_schema']
...
class _RemoteMoECall(torch.autograd.Function):
    ...
    def forward(cls, ctx, expert_logits: torch.Tensor, experts: List[RemoteExpert],
        ...
        expert_args, expert_kwargs = nested_pack(
            flat_inputs, structure=input_schema)
        assert expert_logits.ndim == 1 and len(expert_logits) == len(experts)

        # 1. call experts and await results
        jobs = [partial(cls._run_expert_forward, expert, *
                        expert_args, **expert_kwargs) for expert in experts]
        results = run_and_await_k(
            jobs, k=k_min, timeout_after_k=timeout_after_k_min, timeout_total=timeout_total)
        ...
        ctx.save_for_backward(expert_logits, alive_ix,
                              alive_expert_probs, *stacked_alive_outputs)
    ...
    def backward(cls, ctx, *grad_outputs_flat: torch.Tensor) -> Tuple[Optional[torch.Tensor], ...]:
        ...
        expert_logits, alive_ix, alive_expert_probas, * \
            stacked_alive_outputs = ctx.saved_tensors
        ...
        survived_probas = torch.softmax(
            expert_logits[backward_survivors_ix], dim=0)
        ...
        softmax_jacobian = torch.diagflat(
            survived_probas) - torch.ger(survived_probas, survived_probas)
    ...
    def _run_expert_backward(ctx: EmulatedAutogradContext, weight: torch.Tensor, *grad_outputs: torch.Tensor):
        ...
        grad_dummy, no_grad_uid, no_grad_hostname, no_grad_port, * \
            grad_inputs = backward_result
        return grad_inputs
