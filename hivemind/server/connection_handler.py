...
class ConnectionHandler(mp.Process):
    def __init__(self, addr, port, conn_handler_processes, experts):
        super().__init__()
        self.addr = (addr, port)
...
async def handle_connection(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, thread_pool, thread_pool2, experts):
