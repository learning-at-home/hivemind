...
def serialize_torch_tensor(tensor: torch.Tensor,
                           compression_type=Union[runtime_pb2.CompressionType.NONE, Compression],
    ...
    if not isinstance(compression_type, Compression):
        compression_type = BASE_COMPRESSION_TYPES[runtime_pb2.CompressionType.Name(compression_type)]
...
def deserialize_torch_tensor(serialized_tensor: runtime_pb2.Tensor) -> torch.Tensor:
    ...
    compression_type = BASE_COMPRESSION_TYPES[runtime_pb2.CompressionType.Name(serialized_tensor.compression_type)]
