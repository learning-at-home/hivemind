...
from .routing import BinaryDHTID
from .routing import DHTExpiration
from .routing import DHTID
from .routing import DHTValue
...
class KademliaProtocol(RPCProtocol):
    ...
    def rpc_ping(self, sender: Endpoint,
                 sender_id_bytes: BinaryDHTID) -> BinaryDHTID:
    ...
    async def call_ping(self, recipient: Endpoint) -> Optional[DHTID]:
        ...
        self.routing_table.register_request_to(recipient,
                                               recipient_node_id,
                                               responded=responded)
    ...
    def rpc_store(
            self,
            sender: Endpoint,
            sender_id_bytes: BinaryDHTID,
            key_bytes: BinaryDHTID,
            value: DHTValue,
            expiration_time: DHTExpiration,
    ) -> Tuple[bool, BinaryDHTID]:
        """ Some node wants us to store this (key, value) pair """
        self.routing_table.register_request_from(
            sender, DHTID.from_bytes(sender_id_bytes))
        store_accepted = self.storage.store(DHTID.from_bytes(key_bytes), value,
                                            expiration_time)
    ...
    async def call_store(
        ...
        """
        responded, response = await self.store(recipient, bytes(self.node_id),
                                               bytes(key), value,
                                               expiration_time)
        if responded:
            self.routing_table.register_request_to(recipient,
    ...
    def rpc_find_node(
            self, sender: Endpoint, sender_id_bytes: BinaryDHTID,
            key_bytes: BinaryDHTID
    ) -> Tuple[List[Tuple[BinaryDHTID, Endpoint]], BinaryDHTID]:
        """
        ...
        """
        key_node_id, sender_id = (
            DHTID.from_bytes(key_bytes),
            DHTID.from_bytes(sender_id_bytes),
        )
        self.routing_table.register_request_from(sender, sender_id)
        peer_ids = self.routing_table.get_nearest_neighbors(key_node_id,
                                                            k=self.bucket_size,
                                                            exclude=sender_id)
        return (
            [(bytes(peer_id), self.routing_table[peer_id])
             for peer_id in peer_ids],
            bytes(self.node_id),
        )

    async def call_find_node(self, recipient: Endpoint, key_node_id: DHTID
        ...
        """
        responded, response = await self.find_node(recipient,
                                                   bytes(self.node_id),
                                                   bytes(key_node_id))
        if responded:
    ...
    def rpc_find_value(
            self, sender: Endpoint, sender_id_bytes: BinaryDHTID,
            key_bytes: BinaryDHTID
    ) -> Tuple[Optional[DHTValue], Optional[DHTExpiration], List[
            Tuple[BinaryDHTID, Endpoint]], BinaryDHTID, ]:
        """
        ...
        return self.storage.get(
            DHTID.from_bytes(key_bytes)) + self.rpc_find_node(
                sender, sender_id_bytes, key_bytes)

    async def call_find_value(
            self, recipient: Endpoint, key: BinaryDHTID
    ) -> Tuple[Optional[DHTValue], Optional[DHTExpiration], List[
        ...
        """
        responded, response = await self.find_value(recipient,
                                                    bytes(self.node_id),
                                                    bytes(key))
        if responded:
            ...
            self.routing_table.register_request_to(
                recipient,
                DHTID.from_bytes(recipient_id_bytes),
                responded=responded)
...
class LocalStorage(dict):
    def store(self, key: DHTID, value: DHTValue,
        ...
        return (
            True  # TODO implement actual local storage, test that the logic is correct
        )
