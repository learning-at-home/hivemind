...
class KademliaProtocol(RPCProtocol):
    ...
    async def call_store(
        ...
        """
        responded, response = await self.store(
            recipient, bytes(self.node_id), bytes(key), value, expiration_time
        )
        if responded:
            self.routing_table.register_request_to(
    ...
    def rpc_find_node(
        self, sender: Endpoint, sender_id_bytes: BinaryDHTID, key_bytes: BinaryDHTID
        ...
        return (
            [(bytes(peer_id), self.routing_table[peer_id]) for peer_id in peer_ids],
            bytes(self.node_id),
        )

    async def call_find_node(
        self, recipient: Endpoint, key_node_id: DHTID
        ...
        """
        responded, response = await self.find_node(
            recipient, bytes(self.node_id), bytes(key_node_id)
        )
        if responded:
            peers = [
                (DHTID.from_bytes(peer_id_bytes), tuple(addr))
    ...
    def rpc_find_value(
        ...
        List[Tuple[BinaryDHTID, Endpoint]],
        BinaryDHTID,
    ]:
        """
        ...
        """
        return self.storage.get(DHTID.from_bytes(key_bytes)) + self.rpc_find_node(
    ...
    async def call_find_value(
        ...
        Optional[DHTValue], Optional[DHTExpiration], List[Tuple[DHTID, Endpoint]]
    ]:
        """
        ...
        """
        responded, response = await self.find_value(
            recipient, bytes(self.node_id), bytes(key)
        )
        if responded:
...
class LocalStorage(dict):
    def store(
        self, key: DHTID, value: DHTValue, expiration_time: DHTExpiration
    ) -> bool:
        """
