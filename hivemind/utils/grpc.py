...
def split_for_streaming(serialized_tensor: runtime_pb2.Tensor) -> Iterator[runtime_pb2.Tensor]:
    """ Split serialized_tensor into multiple chunks for gRPC streaming """
    buffer = memoryview(serialized_tensor.buffer)
    num_chunks = len(range(0, len(buffer), STREAMING_CHUNK_SIZE_BYTES))
    yield runtime_pb2.Tensor(
        compression=serialized_tensor.compression, buffer=buffer[:STREAMING_CHUNK_SIZE_BYTES].tobytes(),
        chunks=num_chunks, size=serialized_tensor.size, dtype=serialized_tensor.dtype,
        requires_grad=serialized_tensor.requires_grad)
    for chunk_start in range(STREAMING_CHUNK_SIZE_BYTES, len(buffer), STREAMING_CHUNK_SIZE_BYTES):
        yield runtime_pb2.Tensor(buffer=buffer[chunk_start: chunk_start + STREAMING_CHUNK_SIZE_BYTES].tobytes())
