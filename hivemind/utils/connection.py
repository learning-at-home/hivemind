...
# https://networkengineering.stackexchange.com/a/9435
...
class Connection(AbstractContextManager):
    ...
    def send_raw(self, header: str, content: bytes):
        ...
        self.conn.send(len(content).to_bytes(
            self.payload_length_size, byteorder='big'))
    ...
    def recv_raw(self, max_package: int = 2048) -> bytes:
        length = int.from_bytes(self.conn.recv(
            self.payload_length_size), byteorder='big')
        chunks = []
        bytes_recd = 0
