...
def test_ids_basic():
    ...
    for i in range(100):
        ...
        assert DHTID.xor_distance(
            id1, id1) == DHTID.xor_distance(id2, id2) == 0
        assert DHTID.xor_distance(id1, id2) > 0 or (id1 == id2)
        assert len(PickleSerializer.dumps(id1)) - \
            len(PickleSerializer.dumps(int(id1))) < 40
...
def test_ids_depth():
    for i in range(100):
        ...
        ids_bitstr = [
            "".join(bin(bite)[2:].rjust(8, '0')
                    for bite in uid.to_bytes(20, 'big'))
            for uid in ids
        ]
...
def test_routing_table_basic():
    ...
    for phony_neighbor_port in random.sample(range(10000), 100):
        ...
        routing_table.try_add_node(
            phony_id, ('localhost', phony_neighbor_port))
    ...
    for bucket in routing_table.buckets:
        assert len(
            bucket.replacement_nodes) == 0, "There should be no replacement nodes in a table with 100 entries"
    ...
    for (bucket_size, modulo, min_nbuckets, max_nbuckets) in [
        ...
        for bucket in routing_table.buckets:
            assert len(bucket.replacement_nodes) == 0 or len(
                bucket.nodes_to_addr) <= bucket.size
...
def test_routing_table_search():
    ...
    for phony_neighbor_port in random.sample(range(1_000_000), 10_000):
        num_added += routing_table.try_add_node(
            DHTID.generate(), ('localhost', phony_neighbor_port))
    num_replacements = sum(len(bucket.replacement_nodes)
                           for bucket in routing_table.buckets)
    ...
    for i in range(500):
        ...
        our_knn = routing_table.get_nearest_neighbors(
    ...
    for i in range(500):
        ...
        assert routing_table.get_nearest_neighbors(
            query_id, k=k, exclude=None)[0] == query_id
