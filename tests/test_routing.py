...
def test_ids_basic():
    ...
    for i in range(100):
        ...
        assert (DHTID.from_bytes(bytes(id1)) == id1
...
def test_routing_table_parameters():
    for (bucket_size, modulo, min_nbuckets, max_nbuckets) in [
        ...
        for phony_neighbor_port in random.sample(range(1_000_000), 10_000):
            routing_table.try_add_node(DHTID.generate(),
                                       ("localhost", phony_neighbor_port))
        for bucket in routing_table.buckets:
...
def test_routing_table_search():
    ...
    all_active_neighbors = list(
        chain(*(bucket.nodes_to_addr.keys()
                for bucket in routing_table.buckets)))
    ...
    for i in range(500):
        ...
        our_knn = routing_table.get_nearest_neighbors(query_id,
                                                      k=k,
                                                      exclude=exclude)
    ...
    for i in range(500):
        ...
        reference_knn = heapq.nsmallest(
            ...
            if uid != query_id else float("inf"),
        )
