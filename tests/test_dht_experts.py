...
from hivemind import DHT, RemoteExpert
...
def test_store_get_experts(n_peers=10):
    ...
    peers += [DHT(initial_peers=initial_peers, start=True) for _ in range(n_peers - 1)]
    ...
    assert isinstance(first_found, RemoteExpert)
...
def test_beam_search(
    ...
    dht_instances = [DHT(start=True)]
    initial_peers = dht_instances[0].get_visible_maddrs()
    dht_instances += [DHT(initial_peers=initial_peers, start=True) for _ in range(n_peers - 1)]
    ...
    you = DHT(start=True, initial_peers=neighbors, parallel_rpc=parallel_rpc)
    ...
    for i in range(10):
        ...
        assert all(isinstance(e, RemoteExpert) for e in topk_experts)
    ...
    for i in range(10):
        ...
        assert all(isinstance(e, RemoteExpert) for experts in batch_experts for e in experts)
...
async def test_negative_caching(n_peers=10):
    ...
    peers = [DHT(start=True, **dht_kwargs)]
    initial_peers = peers[0].get_visible_maddrs()
    peers += [DHT(initial_peers=initial_peers, start=True, **dht_kwargs) for _ in range(n_peers - 1)]
    ...
    neg_caching_peer = DHT(initial_peers=neighbors, start=True, **dht_kwargs)
