...
def test_kademlia_protocol():
    try:
        ...
        peer1_proc = mp.Process(target=dht_peer_func, args=(
            peer1_port, peer1_id, peer1_started), daemon=True)
        ...
        peer2_proc = mp.Process(target=dht_peer_func, args=(peer2_port, peer2_id, peer2_started),
        ...
        listen = loop.create_datagram_endpoint(
            protocol, local_addr=('127.0.0.1', port))
        transport, protocol = loop.run_until_complete(listen)
        print(f"Self id={protocol.node_id} port={port}", flush=True)

        assert loop.run_until_complete(protocol.call_ping(
            ('127.0.0.1', peer1_port))) == peer1_id

        key, value, expiration = DHTID.generate(
        ), [123, {'ololo': 'pyshpysh'}], time.monotonic() + 1e3
        assert loop.run_until_complete(protocol.call_store(
            ('127.0.0.1', peer1_port), key, value, expiration))
