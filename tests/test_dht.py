...
def run_protocol_listener(port: int, dhtid: DHTID, started: mp.synchronize.Event, ping: Optional[Endpoint] = None):
    ...
    protocol = DHTProtocol.create(dhtid, bucket_size=20, depth_modulo=5, num_replicas=3, wait_timeout=5,
...
def test_kademlia_protocol():
    try:
        ...
        protocol = DHTProtocol.create(DHTID.generate(), bucket_size=20, depth_modulo=5, wait_timeout=5, num_replicas=3)
        ...
        nodes_found = loop.run_until_complete(protocol.call_find(f'{LOCALHOST}:{peer1_port}', [key]))
        print('!!!', flush=True)
