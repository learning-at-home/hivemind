...
def test_store_maxsize():
    ...
    d.store(DHTID("key1"), "val1", time.monotonic() + 1)
    d.store(DHTID("key2"), "val2", time.monotonic() + 2)
    assert d.get(DHTID("key1")) == (None, None), "elder a value must be deleted"
    assert d.get(DHTID("key2"))[0] == "val2", "Newer should be stored"
