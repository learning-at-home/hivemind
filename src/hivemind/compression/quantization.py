...
class BlockwiseQuantization(Quantization):
    ...
    def extract(self, serialized_tensor: runtime_pb2.Tensor) -> torch.Tensor:
        ...
        absmax_size = int(np.frombuffer(serialized_tensor.buffer, count=1, dtype=np.int64)[0])
        codebook_size = int(np.frombuffer(serialized_tensor.buffer, offset=8, count=1, dtype=np.int64)[0])
